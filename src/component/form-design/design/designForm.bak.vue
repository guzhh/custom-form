					...
					<transition-group name="fade" tag="div">
						<template v-if="element.type === 'collapse'">
							<n-collapse
								...
								:arrow-placement="element.options.arrowPlacement"
								:default-expanded-names="element.options.defaultExpandedNames"
							...
							>
								<n-collapse-item
									:title="item.title"
									:name="item.name"
									v-for="(item, itemIndex) of element.items"
									:key="itemIndex"
								>
									<Draggable
										class="widget-form-list"
										...
										:group="{ name: 'people' }"
										:list="item.list"
										@add="handleItemMoveAdd($event, element, itemIndex)"
									>
										<template #item="{ element, index }">
											<transition-group name="fade" tag="div">
														...
														<n-gi v-for="(col, colIndex) of element.columns" :key="colIndex" :span="col.span ?? 0">
															...
															<div class="widget-view-action widget-col-action" v-if="widgetFormSelect?.key === element.key">
																<SvgIcon
																	iconClass="delete"
																	class-name="delete"
																	@click.stop="handleDeleteClick(itemIndex, item.list)"
																/>
															</div>
												...
												<template v-else>
													<DesignFormItem
														...
														:selectWidget="widgetFormSelect"
														@click.stop="handleItemClick(element)"
														@copy="handleCopyClick(itemIndex, item.list)"
														@delete="handleDeleteClick(itemIndex, item.list)"
													/>
												</template>
											</transition-group>
										</template>
...
const handleMoveAdd = event => {
	...
	if (list[newIndex].type === "collapse") {
		list[newIndex] = {
			...list[newIndex],
			items: list[newIndex].items.map(item => ({ ...item }))
		};
	}
...
const handleColMoveAdd = (event, row, index) => {
	...
	if (row.columns[index].list[newIndex].type === "grid" || row.columns[index].list[newIndex].type === "collapse") {
 ...
 * @param index
 */
const handleItemMoveAdd = (event, row, index) => {
	console.log("-----监听到有新的元素被拖到栅格布局中-------", event, row, index);
	const { newIndex, oldIndex, item } = event;
	// 克隆父组件传递过来的全部表单项
	const list = JSON.parse(JSON.stringify(props.widgetForm.list));

	// 判断拖进来的是折叠面板布局组件还是其他组件，如果是折叠面板组件则不进行接收
	if (row.columns[index].list[newIndex].type === "collapse") {
		// eslint-disable-next-line no-unused-expressions
		item.tagName === "DIV" && list.splice(oldIndex, 0, row.columns[index].list[newIndex]);
		row.columns[index].list.splice(newIndex, 1);
	} else {
		...
		if (list[newIndex].type === "grid") {
			list[newIndex] = {
				...list[newIndex],
				columns: list[newIndex].columns.map(column => ({ ...column }))
			};
		}
